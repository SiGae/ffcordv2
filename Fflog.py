...
class Fflog:
    ...
    async def getToken(cls):
        auth_tuple = (FFLOG_CLIENT, FFLOG_SECRET)
        # print(auth_tuple)
        # print(type(auth_tuple))
...
# fflog 서버 호출
    ...
    async def call_fflog_server(cls, query):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get('https://www.fflogs.com/api/v2/client', headers={'Authorization': 'Bearer {}'.format(cls.token), 'Content-Type': 'application/json'}, json={"query": query}) as resp:
                try:
